...
function main() {
    $html = file_get_contents("http://www.teamliquid.net/calendar/2013/02/?tourney=16");
    $html = str_replace("\n", "", $html);
    //    print_r($html);        
...
function insert_events($date, $name) {
    ...
    $sql = "UPDATE `gazoo`.`event`  SET `description`='http://www.teamliquid.net/calendar/2013/02/?tourney=20' WHERE name='" 
    ...
    $sql = "INSERT INTO `gazoo`.`subject_event` (`id_event`, `id_subject`) VALUES ('" . mysql_insert_id() . "', '100');";
    //$res = mysql_query($sql) OR die(mysql_error());
    }
}
